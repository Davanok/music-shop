...
- Корзина с мгновенным обновлением счетчика, содержимого, удаления и количества товаров без кнопки ручного обновления.
- Оформление заказа: отображаются только товары, доставка и итог.
- История заказов по электронной почте покупателя.
- Публичные вход и регистрация доступны только неадминистративным пользователям.
- Администраторы входят через отдельную страницу `/admin/login` и управляются отдельно.
